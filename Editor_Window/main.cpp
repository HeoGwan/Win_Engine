// IdhEngine.cpp : 애플리케이션에 대한 진입점을 정의합니다.
//
...
#include "..\\IdhEngine_Window\\idhToolScene.h"
                     ...
                     _In_ int       nCmdShow)
{
    ...
    LoadStringW(hInstance, IDC_IDHENGINE, szWindowClass, MAX_LOADSTRING);
    ...
    HACCEL hAccelTable = LoadAccelerators(hInstance, MAKEINTRESOURCE(IDC_IDHENGINE));
    ...
    // 메세지큐에 아무것도 없다면??? 아무 메세지도 가져오기 않게 된다.

    // PeekMessage : 메세지큐에 메세지 유무에 상관없이 함수가 리턴된다.
    //                  리턴 값이 true인 경우 메세지가 있고 false인 경우는 메세지가 없다고 가르쳐준다
...
ATOM MyRegisterClass(HINSTANCE hInstance, const wchar_t* name, WNDPROC proc)
{
    ...
    wcex.hIcon          = LoadIcon(hInstance, MAKEINTRESOURCE(IDI_IDHENGINE));
    wcex.hCursor        = LoadCursor(nullptr, IDC_ARROW);
    wcex.hbrBackground  = (HBRUSH)(COLOR_WINDOW+1);
    wcex.lpszMenuName   = MAKEINTRESOURCEW(IDC_IDHENGINE);
